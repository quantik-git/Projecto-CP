...
\start
     |lcbr(
          lcbr(
               ...
               alice (Fork (l,r)) = max (bob l) (bob r)
          )
     )(
          lcbr(
               bob (Leaf a) = a
          )(
               bob (Fork (l,r)) = min (alice l) (alice r)
          )
     )|
%
\just\equiv{ uncurry, def-x }
%
     |lcbr(
          lcbr(
               alice (Leaf a) = a
...
%
\just\equiv{ natural-id, igualdade extensional }
%
     |lcbr(
          lcbr(
               alice . Leaf = id
...
alice = either id umax . recLTree bob . outLTree

bob :: Ord c => LTree c -> c
bob   = either id umin . recLTree alice . outLTree

both :: Ord d => LTree d -> (d, d)
both  = split alice bob

both' :: Ord d => LTree d -> (d, d)
both' = cataLTree (split a b) where
     a = either id umax . recLTree p2
